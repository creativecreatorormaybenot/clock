...
class _AnimatedTemperatureState
  ...
  void forEachTween(TweenVisitor<dynamic> visitor) {
    _temperature = visitor(_temperature, widget.model.temperature,
        (value) => Tween<double>(begin: value as double)) as Tween<double>;
    _low = visitor(_low, widget.model.low,
        (value) => Tween<double>(begin: value as double)) as Tween<double>;
    _high = visitor(_high, widget.model.high,
        (value) => Tween<double>(begin: value as double)) as Tween<double>;
  }
...
class RenderTemperature extends RenderCompositionChild {
  ...
  void paint(PaintingContext context, Offset offset) {
    ...
    () {
      ...
      canvas.drawOval(
          topRect, Paint()..shader = bradGradient.createShader(topRect));

      final bottomRect = Rect.fromCircle(
          center: Offset(size.width / 2, size.height - bradIndent),
          radius: bradRadius);
      canvas.drawOval(
          bottomRect, Paint()..shader = bradGradient.createShader(bottomRect));
    }();
    //</editor-fold>
    ...
    canvas.drawLine(tube.startOffset(dx: size.width / 2),
        tube.endOffset(dx: size.width / 2), tubePaint);
    //</editor-fold>
    ...
    () {
      ...
      canvas.drawRect(
          startRect, Paint()..shader = bracketGradient.createShader(startRect));

      final endRect = brackets.endOffset(dx: dx) & bracketSize;
      canvas.drawRect(
          endRect, Paint()..shader = bracketGradient.createShader(endRect));
    }();
    //</editor-fold>
  ...
  void _drawLines(Canvas canvas, Line constraints) {
    ...
    final majorValue = unit == TemperatureUnit.fahrenheit ? 20 : 10,
    ...
    final minMax = temperatureScale[unit],
        min = minMax[0],
        max = minMax[1],
    ...
    for (var i = min; i <= max; i++) {
      ...
      if (i % majorValue == 0) {
        final line =
            Line.fromCenter(center: size.width / 2, extent: size.width / 1.46);
        ...
        final line =
            Line.fromCenter(center: size.width / 2, extent: size.width / 3.3);
  ...
  void _drawTemperature(Canvas canvas, Line tube, Line lines,
      double strokeWidth, double temperature, Color color,
      {String text, bool textLeft = true}) {
    final paint = Paint()
      ..color = color
    ...
    final currentScale = temperatureScale[unit],
        ...
        offset = lines.startOffset(dx: size.width / 2) +
            Offset(
                0,
                lines.extent /
                    temperatureRange *
                    (temperatureRange - currentTemperature));
    ...
    final horizontalLine =
        Line.fromCenter(center: offset.dx, extent: strokeWidth);
    canvas.drawLine(
